...
function InsertWord(wordtospeak,wordspoken,accuracy,wordFromBot){
  let id=Math.random().toString(36).substr(2, 9);
  firebase.database().ref('conversation_intermediate/'+id).set({
        id:id,
        wordFromBot:wordFromBot,
...
try {
  ...
  const getAccuracy = (givenWord, wordSpoken,wordFromBot) => {
    ...
    fetch("http://localhost:8080/getAccuracy", {
      ...
      .then(res => {
       ...
       InsertWord(givenWord,wordSpoken,accuracy1,wordFromBot);
  ...
  $('#start-record-btn').on('click', function (e) {
    ...
    setTimeout(() => {
      ...
      getAccuracy(window.wordToSpeak, noteContent,window.wordFromBot);
      noteContent = '';
